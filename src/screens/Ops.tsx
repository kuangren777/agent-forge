...
export function OpsMain() {
  const { role, opsSel, setOpsSel } = useApp();
        ...
        <table className="tbl">
          ...
          <tbody>
            {visible.map(o => (
              <tr
                key={o.name}
                className={o.name === opsSel ? 'sel' : ''}
                style={{ cursor: 'pointer' }}
                onClick={() => setOpsSel(o.name)}
              >
...
export function OpsAside() {
    ...
    return (
      <div className="col pad14 fill gap10">
        ...
        <div
          ...
          style={{ borderColor: 'var(--cap-write)', background: 'var(--cap-write-bg)' }}
        >
          <span className="eyebrow" style={{ color: '#9c5316' }}>待双人确认 · 高风险写操作</span>
          <div className="row between sm">
            <span style={{ color: '#9c5316' }}>发起人</span>
            <span className="mono muted2">hr.li@company</span>
          </div>
          <div className="row between sm">
            <span style={{ color: '#9c5316' }}>对象</span>
            <span className="mono muted2">员工 #2031</span>
          </div>
        </div>
        ...
        <div className="row vcenter gap8 sm">
          <Icon n="check" s={14} c="var(--cap-trusted)" />
          <span className="muted2">mgr.zhao 已批准</span>
          <span className="xs muted">12:01</span>
        </div>
        <div className="row vcenter gap8 sm">
          <Dot k="wait" />
          <span className="muted2">等待第二管理员(你)</span>
        </div>
        <div className="row gap8" style={{ marginTop: 4 }}>
          <Btn sz="sm" k="go" ic="check">确认执行</Btn>
          <Btn sz="sm" k="warn" ic="x">拒绝</Btn>
        </div>
  ...
  return (
    <div className="col pad14 fill gap10">
      ...
      <div className="code">
        {'def '}
        <span className="f">{o.name.replace('.', '_')}_policy</span>
        {'(id, op, kw):\n  '}
        ...
        ? (o.status === 'pending'
          ? (
            <div className="row gap8" style={{ marginTop: 2 }}>
              <Btn sz="sm" k="go" ic="check">批准激活</Btn>
              <Btn sz="sm" ic="sliders">调整</Btn>
              <Btn sz="sm" k="ghost">禁用</Btn>
            </div>
          )
          : (
            <div className="row vcenter gap6 sm" style={{ color: 'var(--cap-trusted)', marginTop: 2 }}>
              <Icon n="check" s={14} c="var(--cap-trusted)" />已激活 · query 自动上线
            </div>
          ))
        : (
